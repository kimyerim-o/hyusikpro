...
<form action="<%=request.getContextPath()%>/review/write.go" method="POST">
 <label for="name">닉네임</label>  
 <input type="text" name="name"><br/><br/>
 <label for="repw">암호</label>
 <input type="password" name="repw"><br/><br/>
<!--  <label for="restno">휴게소번호</label>
    <input type="radio" name="restno" value="1" id="1">1
    <input type="radio" name="restno" value="2" id="2">2
    <input type="radio" name="restno" value="3" id="3">3
    <input type="radio" name="restno" value="4" id="4">4
    <input type="radio" name="restno" value="5" id="5">5
	<input type="radio" name="restno" value="6" id="6">6
    <input type="radio" name="restno" value="7" id="7">7
    <input type="radio" name="restno" value="8" id="8">8
    <input type="radio" name="restno" value="9" id="9">9
    <input type="radio" name="restno" value="10" id="10">10
    <input type="radio" name="restno" value="11" id="11">11
    <input type="radio" name="restno" value="12" id="12">12
    <input type="radio" name="restno" value="13" id="13">13<br/><br/>  -->
    <label for="restname">휴게소 이름</label>
     <select name="restname" id="restname" style="width:50%;height:30px;">
		<option selected>선택하세요</option>
       	<option value="서울만남(부산)휴게소">1.서울 만남의 광장 부산방향
       	</option>
          <option value="죽전(서울)휴게소">2.죽전휴게소 서울방향
          </option>
          <option value="기흥(부산)휴게소">3.기흥휴게소 부산방향
          </option>
          <option value="안성(부산)휴게소">4.안성휴게소 서울방향
          </option>
          <option value="입장거봉포도(서울)휴게소">5.입장거봉포도휴게소 서울방향
          </option>
          <option value="망향(부산)휴게소">6.망향휴게소 부산방향
          </option>
          <option value="천안삼거리(서울)휴게소">7.천안삼거리휴게소 서울방향
          </option>
          <option value="천안호두(부산)휴게소">8.천안호두휴게소 부산방향
          </option>
          <option value="청주(서울)휴게소">9.청주휴게소 서울방향
          </option>
          <option value="옥산(부산)휴게소">10.옥산휴게소 부산방향
          </option>
          <option value="죽암(서울)휴게소">11.죽암휴게소 서울방향
          </option>
          <option value="죽암(부산)휴게소">12.죽암휴게소 부산방향
          </option>
          <option value="신탄진(서울)휴게소">13.신탄진휴게소 서울방향
          </option>
    </select><br/><br/>
...
<a href="<%=request.getContextPath()%>/review/list.go?page=${pageNum}">[${pageNum}]</a> 
</c:forEach>
</c:if>
</footer>
</body>
