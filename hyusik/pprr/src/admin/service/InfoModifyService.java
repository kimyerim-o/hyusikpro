...
public class InfoModifyService {
	...
	public void modify(int restno, RestinfoDto restinfoDto, RestconDto restconDto, BrandDto brandDto, FoodDto foodDto,
		...
		try{
			...
			for(int i=0;i<Connames.length;i++) {
				...
				int c=restconDao.update(conn,restno,conname,conde);
				if(c<1) {
					throw new RuntimeException("fail update restcon");
				}
			}
			...
			for(int i=0;i<Brandnames.length;i++) {
				...
				int b=brandDao.update(conn, restno, brandname, brandde);
				if(b<1) {
					throw new RuntimeException("fail update brand");
				}
			}
			...
			for(int i=0;i<Foodnames.length;i++) {
				...
				int f=foodDao.update(conn, restno,foodname,foodcost,season,foodma,foodde);
				if(f<1) {
					throw new RuntimeException("fail update food");
				}
			}

			oilDao.update(conn,restno,oilDto.getOilname(),oilDto.getOiltel(),oilDto.getGasoline(),oilDto.getDisel(),oilDto.getLpg(),oilDto.getCompany(),oilDto.getAir(),oilDto.getCleaner(),oilDto.getMat());
			...
			for(int i=0;i<Oilconnames.length;i++) {
				...
				int o=oilconDao.update(conn,restno,oilconname,oilconde);
				if(o<1) {
					throw new RuntimeException("fail update oilcon");
				}
			}
			conn.commit();
	...
	public void delete(int restno) {
		...
		try{
			...
			int c=restconDao.delete(conn,restno);
			if(c<1) {
				throw new RuntimeException("fail delete restcon");
			}
			
			int b=brandDao.delete(conn, restno);
			if(b<1) {
				throw new RuntimeException("fail delete brand");
			}
			int f=foodDao.delete(conn, restno);
			if(f<1) {
				throw new RuntimeException("fail delete food");
				}

			int oc=oilconDao.delete(conn,restno);
			if(oc<1) {
				throw new RuntimeException("fail delete oilcon");
			}
			
			int o=oilDao.delete(conn,restno);
			if(o<1) {
				throw new RuntimeException("fail delete food");
			}
			
			int r=restinfoDao.delete(conn,restno);
			if(r<1) {
				throw new RuntimeException("fail delete restinfo");
			}
			conn.commit();
		}catch(SQLException e) {
			JdbcUtil.rollback(conn);
